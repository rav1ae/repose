...
$images = array(); 
if ($folder != ''){
	$images[] = '<img src="/img/back.png" width=150 alt="back">';
}
foreach($files as $file){
	...
	if( !preg_match('/\.(jpg|jpeg|png|gif)$/i', $file)){
		...
		if($file != '..' && $file != '.'){
			...
			$images[] = '<img src="/img/folder.png" width=150 alt="folder" title="'.$file.'">';
		}
		
	}
	else{
		$thumbSrc =$_SERVER['DOCUMENT_ROOT'].'/media/pages/' .$folder.'/'. $file;  
		$fileBaseName = str_replace('_th.','.',$file);  
		$imgSrc = '/media/pages/' .$folder.'/'. $file; 
...
tbl.onclick =   
    function(e) {  
        ...
        if( tgt.nodeName != 'IMG' )  
            return;  
		if(tgt.alt == "image"){
			url = '/media/pages/' + folder + '/' + tgt.title;  
			this.onclick = null;  
