<?php
	...
	if(isset($_POST['csrfmiddlewaretoken'])/*  && has_rights("add_nav") */){
		...
		$stt->execute();
		$stt = $db->prepare('INSERT INTO content_nav (block_id, page_id, title, href, is_active, priority) VALUES (?, ?, ?, ?, ?, ?)');
		...
		if(isset($_POST['_continue'])){
			Header('Location: /admin/content/nav/item/?nav='.$insId);
		}
...
<html lang="en-us" >
	<head>
		<title>Добавить элемент меню | Административный сайт YOG.KZ</title>
							...
							<fieldset class="module aligned ">
								<div class="form-row field-block">
									<div>
										...
										<select id="id_block" name="block">
											...
											<option value="<?php echo $navblock[$i][0]?>"><?php echo $navblock[$i][1]?></option>
											<?php endfor;?>
										</select>
									</div>
								</div>
								<div class="form-row field-page">
									<div>
										...
										<select id="id_page" name="page">
											...
											<option value="<?php echo $navpages[$i][0]?>"><?php echo $navpages[$i][1]?></option>
											<?php endfor;?>
										</select>
									</div>
								</div>
								...
								<div class="form-row field-priority">
									<div>
										...
										<input class="vIntegerField" id="id_priority" name="priority" type="text" />
							...
							<script type="text/javascript">
								(function($) {
									$(document).ready(function() {
										$('form#nav_form :input:visible:enabled:first').focus()
									});
								})(django.jQuery);
							</script>
							<script type="text/javascript">
