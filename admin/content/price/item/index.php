<?php
	...
	$itemId = filter_input(INPUT_GET, "price", FILTER_SANITIZE_NUMBER_INT);
	if(isset($_POST['csrfmiddlewaretoken'])){
		...
		$stt = $db->prepare('UPDATE content_price SET abonement = ?, price = ?, to_students = ?, prolong = ? WHERE id = ?');
		$stt->bindParam(1, $abonement);
		$stt->bindParam(2, $price);
		$stt->bindParam(3, $to_students);
		$stt->bindParam(4, $prolong);
		$stt->bindParam(5, $itemId);
		$stt->execute();
		if(isset($_POST['_save'])){
			Header('Location: /admin/content/price/');
		}
		else if(isset($_POST['_addanother'])){
			Header('Location: /admin/content/price/add/');
		}
	}	

	$stt = $db->prepare('SELECT id, abonement, price, to_students, prolong FROM content_price WHERE id = ?');
...
<html lang="en-us" >
	<head>
		<title>Изменить прайс | Административный сайт YOG.KZ</title>
	...
	<body class=" app-content model-news change-form">
		...
		<div id="container">
			...
			<div class="breadcrumbs">
				...
				&rsaquo; <a href="/admin/content/price/">Прайс</a>
				&rsaquo; <?php echo $price[1];?>
			</div>
			<!-- Content -->
			<div id="content" class="colM">
				<h1>Изменить прайс</h1>
				<div id="content-main">
							...
							<fieldset class="module aligned ">
								<div class="form-row field-subtitle">
									<div>
										...
										<input class="vTextField" id="abonement" maxlength="100" name="abonement" type="text"  value="<?php echo $price[1];?>"/>
									</div>
								</div>
								<div class="form-row field-slug">
									<div>
										<label class="required" for="price">Стоимость:</label>
										<input class="vTextField" id="price" maxlength="100" name="price" type="text"  value="<?php echo $price[2];?>"/>
									</div>
								</div>
								<div class="form-row field-url">
									<div>
										<label class="required" for="to_students">Пенсионерам, студентам:</label>
										<input class="vTextField" id="to_students" maxlength="100" name="to_students" type="text"  value="<?php echo $price[3];?>"/>
									</div>
								</div>
								<div class="form-row field-meta">
									<div>
										<label class="required" for="prolong">Продление:</label>
										<input class="vTextField" id="prolong" maxlength="100" name="prolong" type="text"  value="<?php echo $price[4];?>"/>
