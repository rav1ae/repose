	...
	if(isset($_POST['_selected_action'])){
		if($_POST['action'] == 'delete_selected'){
			...
			for($i=0;$i<count($_POST['_selected_action']);$i++){
				...
				$stt = $db->prepare('SELECT picture FROM content_staff WHERE id = ?');
			...
			$stt = $db->prepare('DELETE FROM content_staff WHERE id IN ('.$delCort.')');
			$stt->execute();
		}
	}

		$stt = $db->prepare('SELECT id, first_name, second_name, last_name, is_active, priority FROM content_staff');
		$stt->execute();
...
<html lang="en-us" >
	<head>
		<title>Выберите персонал для изменения | Административный сайт YOG.KZ</title>
	...
	<body class=" app-content model-news change-list">
		...
		<div id="container">
			...
			<div id="content" class="flex">
				<h1>Выберите персонал для изменения</h1>
				<div id="content-main">
					<ul class="object-tools">
						<li>
							<a href="/admin/content/staff/add/" class="addlink">
								Добавить персонал
							</a>
						</li>
					</ul>
						...
						<form id="changelist-form" action="" method="post" novalidate><input type='hidden' name='csrfmiddlewaretoken' value='kydoYqjOKkNwWiLVSQXuC4QFHXz8rO7S' />
							<div class="actions">
								<label>Действие: <select name="action">
									...
									<option value="delete_selected">Удалить выбранный персонал</option>
								...
								<span class="action-counter">Выбрано 0 объектов из <?php echo count($staff);?> </span>
							</div>
							<div class="results">
								<table id="result_list">
									<thead>
										<tr>
											...
											<th scope="col"  class="sortable column-title">
												<div class="text"><a href="?o=1.-3">ФИО</a></div>
												<div class="clear"></div>
											</th>
											...
											<th scope="col"  class="sortable column-is_priority">
												<div class="text"><a href="?o=2.-3">Приоритет</a></div>
												<div class="clear"></div>
											</th>
										</tr>
									</thead>
									<tbody>
										...
										<tr class="<?php echo (($i%2)>0?'row2':'row1')?>">
											<td class="action-checkbox"><input class="action-select" name="_selected_action[]" type="checkbox" value="<?php echo $staff[$i][0]?>" /></td>
											
											<th class="field-title"><a href="/admin/content/staff/item/?item=<?php echo $staff[$i][0]?>"><?php echo $staff[$i][2]?> <?php echo $staff[$i][1]?> <?php echo $staff[$i][3]?></a></th>
											
											<td class="field-is_active"><img src="/img/<?php echo ($staff[$i][4] > 0 ?'icon-yes.gif':'icon-no.gif');?>" alt="True" /></td>
											<td class="field-title"><?php echo $staff[$i][5]?></td>
										</tr>
										<?php endfor;?>
									</tbody>
								</table>
