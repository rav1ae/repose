...
<?php
	$stt = $db->prepare('SELECT title, content, (SELECT slug from content_pages WHERE id = link) FROM content_block LIMIT 2');
	$stt->execute();
...
<div id="second-block" style="background-position-y: -41px;position:relative">
	<div class="container">
		<div class="row">
			...
			<div class="col-md-6 col-sm-12 col-xs-12 inner-second-<?php echo ($i==0?"first":"second")?>" style="">
				<div class="inner-second">
					<h2 style="text-align:center"><?php echo $block[$i][0]?></h2>
					<?php echo $block[$i][1]?>
<span style="float:right;position:relative;bottom:20px"><a href="/page/?p=<?php echo $block[$i][2]?>" class="btn btn-danger btn-xs"><strong>>>>>>>>></strong></a></span>
				</div>
			</div>
			<?php endfor;?>
...
<div id="third-block" style="height:600px;overflow:hidden;">
